...
## 概要
飲食店の従業員向けの注文管理アプリです。
注文を取るページと注文状況を管理できるページです。
メニューの管理と注文のステータス管理ができます。  
Windows での開発からはじまり、ノートPC:8gb ram 512gb ssd(debian13+xfce)への環境移行、Docker によるコンテナ化、AWS EC2 への本番デプロイ、CI/CD パイプラインの構築まで一人で取り組んだプロジェクトです。
...
### 注文管理画面
![注文一覧](./docs/images/order-list.png)

### メニュー一覧画面
![メニュー一覧画面](./docs/images/menu-list.png)
...
- **DB設計の正規化：** メニューの価格改定が過去の注文履歴に影響しないよう、注文時点の`menuName`・`price`を`Order`側にもスナップショットとして保持しています。
...
- [ ] 無停止デプロイ（ゼロダウンタイムデプロイ）の実装（現状、デプロイ中の数秒〜十数秒はアクセス不可になる課題を確認済み）
